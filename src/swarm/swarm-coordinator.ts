...
export class SwarmCoordinator {
  ...
  constructor(
    ...
    if (!isEnabled('SWARM_COORDINATION')) {
      throw new Error(
        '[SwarmCoordinator] SWARM_COORDINATION feature is not enabled. ' +
          'Set FEATURE_SWARM_COORDINATION=true (requires AGENT_POOL and MULTI_LLM).'
      );
    }
  ...
  async execute(userRequest: string, conversationId: string): Promise<SwarmSession> {
    ...
    try {
      ...
      const results = await this.executeGraph(executionGraph, swarmId, session);
