...
`;
      ...
      try {
        ...
        if (chains.length > 0) {
          msg += `## ⛓️ AVAILABLE CHAINS (${String(chains.length)})\n\n`;
          for (const chain of chains.sort()) {
            msg += `• \`${chain}\`\n`;
          }
          msg += '\n';
      ...
      try {
        ...
        if (prompts.length > 0) {
          msg += `## 📝 AVAILABLE PROMPTS (${String(prompts.length)})\n\n`;
          for (const prompt of prompts.sort()) {
            msg += `• \`${prompt}\`\n`;
          }
          msg += '\n';
      ...
      if (dbTemplates.length > 0) {
        msg += `## 🗃️ DATABASE TEMPLATES (${String(dbTemplates.length)})\n\n`;
      ...
      if (conversation.codebase_id) {
        ...
        if (cmdCount > 0) {
          msg += `## 📦 CODEBASE COMMANDS (${String(cmdCount)})\n\n`;
  ...
  /verbose [on|off] - Toggle verbose logging`,
    ...
    case 'verbose': {
      ...
      if (args.length === 0) {
        ...
        return {
          success: true,
          message: `Verbose logging ${newState ? '**ENABLED** ✅' : '**DISABLED** ❌'}\n\nView logs with:\n\`docker-compose --profile with-db logs -f app-with-db\``,
        };
      } else if (args[0] === 'on') {
        setVerbose(true);
        return {
        ...
        return {
          success: true,
          message: `Verbose logging is currently ${status ? '**ENABLED** ✅' : '**DISABLED** ❌'}`,
        };
      } else {
        return {
          success: false,
