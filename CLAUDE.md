...
# Phone Vibecoding V1 - Approval Workflow
BLOCKING_APPROVALS=true      # Enable blocking approvals via Telegram (default: false)
APPROVAL_TIMEOUT_MS=300000   # Approval timeout in ms (default: 5 minutes)
NOTIFY_ON_RISK_TOOLS=true    # Send notifications for high-risk tools (default: true)
```
...
### Multi-Agent Pool (5x Parallel Execution)

**FEAT-003** enables 3-12 Claude agents working in parallel for 5x faster vibe coding. Built on **PostgreSQL NOTIFY/LISTEN** (no Redis required).

**Architecture:**
```
User Message → Coordinator → Task Queue (PostgreSQL)
                    ↓
    ┌───────────┬────────────┬────────────┬───────────┐
  Agent-1    Agent-2     Agent-3     Agent-4
   (idle)     (busy)      (busy)      (idle)
    ↓           ↓           ↓           ↓
 Task A      Task B      Task C    [waiting]
    ↓           ↓           ↓
 Results → Pool Coordinator → Stream to User
```

**Components:**
1. **PoolCoordinator** (`src/pool/pool-coordinator.ts`) - Task distribution, result collection
2. **AgentWorker** (`src/pool/agent-worker.ts`) - Picks up tasks, executes Claude SDK
3. **TaskQueue** (`src/pool/task-queue.ts`) - Priority queue (1=highest → 10=lowest)
4. **AgentRegistry** (`src/pool/agent-registry.ts`) - Health monitoring, stale detection
5. **PgPubSub** (`src/pool/pubsub.ts`) - PostgreSQL NOTIFY/LISTEN wrapper

**Database Tables:**
- `agent_pool` - Active agents (status, heartbeats, capabilities)
- `pool_tasks` - Priority task queue
- `pool_task_results` - Streaming results
...
AGENT_POOL_SIZE=4              # 3-12 agents
AGENT_HEARTBEAT_INTERVAL=30000 # 30s
AGENT_STALE_THRESHOLD=120      # 2min before agent considered dead
AGENT_TASK_TIMEOUT=300         # 5min before reassigning stuck tasks
...
# Should show 4 agents with status='idle' and recent heartbeats
```

**How Workers Auto-Start:**

The `scripts/start-with-workers.sh` script (called by Docker CMD):
1. Checks `FEATURE_AGENT_POOL` flag
2. Spawns `AGENT_POOL_SIZE` worker processes in background
3. Workers auto-restart on crash
4. Main app runs in foreground
5. Graceful shutdown on container stop

**Manual Worker Control:**

```bash
# Start single worker manually (for debugging)
docker compose --profile with-db exec app-with-db bun src/scripts/start-worker.ts --agent-id=debug-worker

# View worker logs
docker compose --profile with-db logs -f app-with-db | grep "\[Worker\]"
```
...
docker compose --profile with-db exec postgres psql -U postgres -d lugh -c "
SELECT
  (SELECT COUNT(*) FROM agent_pool WHERE status='idle') as idle,
  (SELECT COUNT(*) FROM agent_pool WHERE status='busy') as busy,
  (SELECT COUNT(*) FROM pool_tasks WHERE status='queued') as queued,
  (SELECT COUNT(*) FROM pool_tasks WHERE status='completed') as completed;
"

# Recent tasks
docker compose --profile with-db exec postgres psql -U postgres -d lugh -c "
SELECT id, task_type, status, priority, assigned_agent_id, created_at
FROM pool_tasks ORDER BY created_at DESC LIMIT 10;
"
```
...
|--------|-------|------------|
| Infrastructure | +1 service | Already have it |
| Pub/Sub | Redis channels | NOTIFY/LISTEN |
| Latency | <1ms | 5-50ms (fine for 3-12 agents) |
| Data Persistence | Manual (RDB/AOF) | Automatic (ACID) |
| Complexity | High | Low |

**For 3-12 agents, PostgreSQL is perfect.** Only need Redis when scaling to 50+ agents (see `FEAT-019`).

**Troubleshooting:**

```bash
# Agents stuck offline?
# Force cleanup and restart container
docker compose --profile with-db exec postgres psql -U postgres -d lugh -c "DELETE FROM agent_pool;"
docker compose --profile with-db restart app-with-db

# Tasks stuck in queue?
# Check agent logs
docker compose --profile with-db logs --tail=50 app-with-db | grep -E "\[Agent|\[Worker\]"

# Disable agent pool temporarily
# Set in .env: FEATURE_AGENT_POOL=false
# Restart: docker compose --profile with-db restart app-with-db
```

**Performance:**
- **Latency**: Task assignment < 100ms
- **Throughput**: 4 agents = 4x parallel execution
- **Scalability**: PostgreSQL handles 12 agents easily
- **Reliability**: Auto-restart, stale detection, task reassignment

**See also:**
- Full spec: `features/FEAT-003-multi-agent-no-redis.md`
- Testing guide: `FEAT-003-TESTING.md`
- Integration tests: `src/pool/integration.test.ts`
